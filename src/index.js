...
async function registerCommands() {
  ...
  try {
    if (process.env.GUILD_ID) {
      ...
      console.log('✅ Global commands geregistreerd');
    }
  } catch (e) {
    console.error('❌ Command-registratie fout:', e);
  }
}
...
async function startStatusLoop() {
  try {
    ...
    if (!channel || !channel.isTextBased()) {
      console.warn('⚠️ Status-kanaal niet gevonden of geen tekstkanaal.');
      return;
    }
    ...
    statusInterval = setInterval(async () => {
      try {
        const embed = buildStatusEmbed(await getGuildName());
...
client.on('guildCreate', refreshPresence);
client.on('guildDelete', refreshPresence);
...
client.on('guildMemberAdd', async (member) => {
  try {
    ...
    if (assignable.length) await member.roles.add(assignable.map(r => r.id), 'Autorole bij join');
  } catch (e) {
    ...
    switch (ix.commandName) {
      case 'add-autorole': {
        ...
        if (!canManageRole(ix.guild, role, me))
          return ix.reply(err('Ik kan deze rol niet beheren.'));
        const added = await addAutorole(ix.guild.id, role.id);
        return ix.reply(added ? ok(`Rol **${role.name}** toegevoegd aan autorole-lijst.`)
                              : warn(`Rol **${role.name}** stond al in de lijst.`));
      }
      case 'remove-autorole': {
        if (!needManageRoles(invoker)) return ix.reply(err('Je mist **Manage Roles**.'));
        const role = ix.options.getRole('rol', true);
        const removed = await removeAutorole(ix.guild.id, role.id);
        return ix.reply(removed ? ok(`Rol **${role.name}** verwijderd uit autorole-lijst.`)
                                : warn(`Rol **${role.name}** stond niet in de lijst.`));
      }
      case 'add-role': {
        if (!needManageRoles(invoker)) return ix.reply(err('Je mist **Manage Roles**.'));
